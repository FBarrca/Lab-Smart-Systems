...
uint64_t helpers::CTimeUtils::seconds_from_epoch(const boost::posix_time::ptime &date, const bool &isAlreadyConvertedToUtc /*= false*/)
{
	...
	if (!isAlreadyConvertedToUtc)
		seconds -= getLocalAndUniversalTimeDifferential(date).total_seconds();
	return seconds;
}
...
time_t helpers::CTimeUtils::getTimeTFromYMDHMS(const int &y, const int &m, const int &d, const int &h, const int &min, const int &s)
{
	struct tm tm;
	tm.tm_year = y - 1900;
	tm.tm_mon = m - 1;
