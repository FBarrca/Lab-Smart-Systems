...
bool CDatabaseMIC::updateValue(const int32_t& IdVariable, const CValue& value)
{
	...
	if (!this->getValues(IdVariable,vCValue)) {
		_log.println(boost::log::trivial::info, "cant get values");
		return false;
	}
	CValue findResult;
	if (!findCValueOnVectorByDate(vCValue, findResult, value.getDate()))
		_log.println(boost::log::trivial::info,  "UPDATE: We looked for value of that id on that time but found no data");
	else
		_log.println(boost::log::trivial::info,  "UPDATE: Found inserted data at that time");
